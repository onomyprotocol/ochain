...
The main command used to set up `ochaind` is the following:
...
- `cosmos`
  - Derived from account keys generated by `ochaind keys add`
...
- `cosmosvalconspub`
  - Generated when the node is created with `ochaind init`.
  - Get this value with `ochaind tendermint show-validator`
...
Older versions of `ochaind` used store keys in the user's home directory. If you are migrating
from an old version of `ochaind` you will need to migrate your old keys into your operating system's
credentials storage by running the following command:

```bash
ochaind keys migrate
```
...
When broadcasting transactions, `ochaind` accepts a `--broadcast-mode` flag. This
...
```bash
ochaind tx send <sender_key_name_or_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id>
```
...
```bash
ochaind tx send <sender_key_name_or_address> <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
ochaind tx send <sender_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id> \
...
::: tip Note
The `--generate-only` flag prevents `ochaind` from accessing the local keybase.
...
where `[name]` is the name of the key you specified when you initialized `ochaind`.
...
out the Governance module [specification](https://github.com/onomyprotocol/onomy-sdk/tree/master/x/gov/spec).
...
```bash
ochaind query distribution community-pool
```
...
```bash
ochaind tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 1000000uatom \
