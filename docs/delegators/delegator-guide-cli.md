...
This document contains all the necessary information for delegators to interact with the Onomy through the Command-Line Interface (CLI).
...
Please also note that you are about to interact with the Onomy, a
...
`ochaind`: This is the command-line interface to interact with a `ochaind` full-node. 

::: warning
**Please check that you download the latest stable release of `ochaind` that is available**
:::

[**Download the binaries**]
Not available yet.

[**Install from source**](../ochain-tutorials/installation.md)

::: tip
`ochaind` is used from a terminal. To open the terminal, follow these steps:
...
```bash
ochaind keys add <yourKeyName> --recover
```
...
To create an account, you just need to have `ochaind` installed. Before creating it, you need to know where you intend to store and interact with your private keys. The best options are to store them in an offline dedicated computer or a ledger device. Storing them on your regular online computer involves more risk, since anyone who infiltrates your computer through the internet could exfiltrate your private keys and steal your funds.
...
When you initialize your ledger, a 24-word mnemonic is generated and stored in the device. This mnemonic is compatible with Cosmos and Cosmos accounts can be derived from it. Therefore, all you have to do is make your ledger compatible with `ochaind`. To do so, you need to go through the following steps:
...
```bash
ochaind keys add <yourAccountName> --ledger 
```
...
```bash
ochaind keys add <yourKeyName> --recover --account 1
```

This command will prompt you to input a passphrase as well as your mnemonic. Change the account number to generate a different account. 


## Accessing the Onomy Network
...
You will find the tutorial on how to install `ochaind` [here](https://cosmos.network/docs/cosmos-hub/installation.html), and the guide to run a full-node [here](https://cosmos.network/docs/cosmos-hub/join-mainnet.html).

### Connecting to a Remote Full-Node

If you do not want or cannot run your own node, you can connect to someone else's full-node. You should pick an operator you trust, because a malicious operator could return  incorrect query results or censor your transactions. However, they will never be able to steal your funds, as your private keys are stored locally on your computer or ledger device. Possible options of full-node operators include validators, wallet providers or exchanges. 

In order to connect to the full-node, you will need an address of the following form: `https://77.87.106.33:26657` (*Note: This is a placeholder*). This address has to be communicated by the full-node operator you choose to trust. You will use this address in the [following section](#setting-up-ochaind).
...
::: tip
**Before you can bond atoms and withdraw rewards, you need to [set up `ochaind`](#setting-up-ochaind)**
:::

`ochaind` lets you query all relevant information from the blockchain, like account balances, amount of bonded tokens, outstanding rewards, governance proposals and more. Next is a list of the most useful commands for delegator. 

```bash
// query account balances and other account-related information
ochaind query account <yourAddress>
...
ochaind query staking delegation <delegatorAddress> <validatorAddress>

// query the rewards of a delegator given a delegator address (e.g. cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg)
ochaind query distribution rewards <delegatorAddress> 

// query all proposals currently open for depositing
ochaind query gov proposals --status deposit_period

// query all proposals currently open for voting
ochaind query gov proposals --status voting_period

// query a proposal given its proposalID
ochaind query gov proposal <proposalID>
```
...
::: tip
**Before you can bond atoms and withdraw rewards, you need to [set up `ochaind`](#setting-up-ochaind) and [create an account](#creating-an-account)**
:::
...
ochaind tx send <to_address> <amount> --from <yourKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
```
...
ochaind tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
...
ochaind tx staking redelegate <srcValidatorAddress> <destValidatorAddress> <amountToRedelegate> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>

// Withdraw all rewards
// ex value for flag: <gasPrice>=0.025uatom

ochaind tx distribution withdraw-all-rewards --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
...
ochaind tx staking unbond <validatorAddress> <amountToUnbond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
```
...
The Onomy has a built-in governance system that lets bonded Atom holders vote on proposals. There are three types of proposal:
...
ochaind tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <type> --deposit=10000000uatom --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Increase deposit of a proposal
// Retrieve proposalID from $ochaind query gov proposals --status deposit_period
// ex value for parameter: <deposit>=10000000uatom

ochaind tx gov deposit <proposalID> <deposit> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Vote on a proposal
// Retrieve proposalID from $ochaind query gov proposals --status voting_period 
// <option>=yes/no/no_with_veto/abstain

ochaind tx gov vote <proposalID> <option> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>
```
...
ochaind tx staking delegate <validatorAddress> <amountToBond> --from <delegatorAddress> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --generate-only > unsignedTX.json
```
...
```bash
ochaind query account <yourAddress> --chain-id cosmoshub-2
```
...
```bash
ochaind tx sign unsignedTx.json --from <delegatorKeyName> --offline --chain-id cosmoshub-2 --sequence <sequence> --account-number <account-number> > signedTx.json
```
