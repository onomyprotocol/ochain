...
This document describes the upgrade procedure of a `ochaind` full-node to a new version.

## Software Upgrade

First, stop your instance of `ochaind`. Next, upgrade the software:
...
See the [testnet repo](https://github.com/cosmos/testnets) for details on which version is needed for which public testnet, and the [Ochain release page](https://github.com/cosmos/Ochain/releases) for details on each release.
...
```bash
cd $HOME/.ochain/config
ochaind export --for-zero-height --height=<export-height> > new_genesis.json
```
...
::: warning
If you are running a **validator node** on the mainnet, always be careful when doing `ochaind unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
