...
1. Start the network and trigger upgrade
   ...
   ```bash
   # start a ochain application full-node
   $ ochaind start

   # set up the cli config
   $ ochaind config trust-node true
   $ ochaind config chain-id testing

   # create an upgrade governance proposal
   $ ochaind tx gov submit-proposal software-upgrade <plan-name> \
...
2. Performing an upgrade
   ...
   manually kill the process and replace it with a new binary. Do so now with `Ctrl+C` or `killall ochaind`.

   In `ochain/app/app.go`, after `upgrade.Keeper` is initialized and set in the app, set the the
   ...
   ```bash
   # create a new binary of ochain with the added upgrade handler
   $ make install
   ...
   $ ochaind query upgrade applied <plan-name>
   ```
