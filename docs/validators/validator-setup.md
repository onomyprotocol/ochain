...
Before setting up your validator node, make sure you've already gone through the [Full Node Setup](../ochain-tutorials/join-mainnet.md) guide.
...
You may want to skip the next section if you have already [set up a full-node](../ochain-tutorials/join-mainnet.md).
...
Next, craft your `ochaind gentx` command. 
...
you want your node to shutdown or by passing the `--halt-height` flag to `ochaind`.
...
If you got jailed for downtime, you can get your voting power back to your validator. First, if `ochaind` is not running, start it up again:
...
### Problem #2: My `ochaind` crashes because of `too many open files`

The default number of files Linux can open (per-process) is `1024`. `ochaind` is known to open more than `1024` files. This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed) and then restart the process with `ochaind start`. If you are using `systemd` or another process manager to launch `ochaind` this may require some configuration at that level. A sample `systemd` file to fix this issue is below:
