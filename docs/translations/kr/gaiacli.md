...
`ochaind`는 사용자가 풀노드를 통해 코스모스 허브 네트워크를 사용할 수 있게하는 도구입니다. 풀노드는 사용자가 직접 운영하거나 원격 풀노드에 연결하여 사용하실 수 있습니다. `ochaind`를 설치하기 위해서는 [다음 과정](./installation.md)을 참고하세요.

### ochaind 설정하기

`ochaind`를 설정하는데 사용되는 주 명령어는 다음과 같습니다:

```bash
ochaind config <플래그(flag)> <값(value)>
```
...
```bash
ochaind config node <호스트(host)>:<포트(port)>

# 예시: ochaind config node https://77.87.106.33:26657
```
...
```bash
ochaind config trust-node true
...
- `cosmosvalconspub`
  - `ochaind init`로 새로운 노드가 생성될때 같이 생성되는 키.
  - `ochaind tendermint show-validator` 명령으로 키 값을 확인할 수 있음
...
```bash
ochaind keys add <계정_명칭(account_name)>
```
...
```bash
ochaind keys add --recover
```

이제 프라이빗 키를 확인하고 `<계정_명칭(account_name)>`을 찾으면 됩니다:

```bash
ochaind keys show <계정_명칭(account_name)>
```

검증인 운영자 주소는 다음과 같이 확인하실 수 있습니다:

```bash
ochaind keys show <account_name> --bech=val
```
...
```bash
ochaind tendermint show-validator
```
...
```bash
ochaind keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
ochaind keys add --multisig=foo,bar,baz --multisig-threshold=2 multisig_address
ochaind keys add --multisig=baz,foo,bar --multisig-threshold=2 multisig_address
```

멀티시그 키의 주소는 다음과 같이 빠르게 생성하여 커맨드라인에 프린트할 수 있습니다:

```bash
ochaind keys show --multisig-threshold K name1 name2 name3 [...]
```
...
트랜잭션을 전파할때 `ochaind`에서 `--broadcast-mode`를 사용하실 수 있습니다. `--broadcast-mode`는 `sync`(동기, 기본 값), `async`(비동기), 또는 `block`(보류) 값으로 설정될 수 있습니다. `sync` 값은 클라이언트가 CheckTx 리턴 값을 돌려주고, `async` 값은 클라이언트가 즉시 반응을 알려주며, `block` 값은 트랜잭션이 커밋(또는 타임이웃)될 때까지 대기합니다.
...
```bash
ochaind tx send ... --fees=5000uatom
```

또는

```bash
ochaind tx send ... --gas-prices=0.025uatom
```
...
```bash
ochaind query account <코스모스_주소(account_cosmos)>
```
...
```bash
ochaind tx send <보내는_사람_키_명칭_또는_주소(sender_key_name_or_address)> 10faucetToken \
...
```bash
ochaind query account <보내는_주소(account_cosmos)>
ochaind query account <수신자_주소(destination_cosmos)>
```

특정 블록 높의에서의 잔고를 확인하고 싶으시다면 `--block` 플래그를 사용하실 수 있습니다:

```bash
ochaind query account <코스모스_주소(account_cosmos)> --block=<블록_높이(block_height)>
```
...
```bash
ochaind tx send <보내는이_주소(sender_address)> <수신자_주소(destination_cosmosaccaddr)> 10faucetToken \
  --chain-id=<체인_아이디(chain-id)> \
  --generate-only > unsignedSendTx.json
```

::: tip 참고
`--generate-only` 명령어는 `ochaind`가 로컬 키베이스를 액세스하지 않습니다. `--generate-only` 플래그를 사용하시는 경우, `<보내는_사람_키_명칭_또는_주소(sender_key_name_or_address)>` 값은 키 명칭이 아닌 주소 값을 입력하세요.
:::
...
```bash
ochaind tx sign --validate-signatures
  signedSendTx.json
```
...
```bash
ochaind tx broadcast --node=<node> signedSendTx.json
```
...
```bash
ochaind query txs --events='message.sender=cosmos1...'
```

다수의 `events`를 이용하실 경우:

```bash
ochaind query txs --events='message.sender=cosmos1...&message.action=withdraw_delegator_reward'
```

페이지네이션은 `page`와 `limit` 값으로 지원됩니다.

```bash
ochaind query txs --events='message.sender=cosmos1...' --page=1 --limit=20
```
...
- [Staking events](https://github.com/onomyprotocol/onomy-sdk/blob/master/x/staking/spec/07_events.md)
- [Governance events](https://github.com/onomyprotocol/onomy-sdk/blob/master/x/gov/spec/04_events.md)
- [Slashing events](https://github.com/onomyprotocol/onomy-sdk/blob/master/x/slashing/spec/06_events.md)
- [Distribution events](https://github.com/onomyprotocol/onomy-sdk/blob/master/x/distribution/spec/06_events.md)
- [Bank events](https://github.com/onomyprotocol/onomy-sdk/blob/master/x/bank/spec/04_events.md)
:::
...
```bash
ochaind tx slashing unjail --from <검증인_오퍼레이터_주소(validator-operator-addr)>
```
...
```bash
ochaind query slashing signing-info <검증인_pubkey(validator-pubkey)>
```
...
```bash
ochaind query minting params
```

현재 인플레이션 값은 다음과 같이 조회하실 수 있습니다:

```bash
ochaind query minting inflation
```

현재 프로비젼(provisions) 값은 다음과 같이 조회하실 수 있습니다:

```bash
ochaind query minting annual-provisions
```
...
```bash
ochaind query staking validator <cosmosval_계정(account_cosmosval)>
```
...
```bash
ochaind keys show [name] --bech val
```

여기에서`[name]`은 `ochaind`를 처음 설정하셨을때 정의한 키의 명칭입니다.
...
```bash
ochaind query staking delegation <위임자_코스모스_주소(delegator_addr)> <검증인_주소(validator_addr)>
...
```bash
ochaind query staking delegation <위임자_코스모스_주소(delegator_addr)>
```
...
```bash
ochaind query staking unbonding-delegation <위임자_주소(delegator-addr)> <검증인_주소(validator_addr)>
```

또는 특정 위임자의 모든 언본딩 정보를 확인하고 싶으신 경우:

```bash
ochaind query staking unbonding-delegations <위임자_주소(account_cosmos)>
```

추가적으로 특정 검증인으로 부터 언본딩하는 정보를 확인하고 싶으신 경우:

```bash
ochaind query staking unbonding-delegations-from <검증인_cosmosval_주소(account_cosmosval)>
```
...
```bash
ochaind query staking redelegation <위임자_주소(delegator_addr)> <기존_검증인_주소(src_val_addr)> <대상_검증인_주소(dst_val_addr)>
```

특정 위임자의 모든 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
ochaind query staking redelegations <위임자_주소(account_cosmos)>
```

특정 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
  ochaind query staking redelegations-from <검증인_주소(account_cosmosval)>
```
...
```bash
  ochaind query delegations-to <검증인_주소(account_cosmosval)>
```
...
```bash
ochaind tx gov submit-proposal \
  --title=<제목(title)> \
...
```bash
ochaind tx gov submit-proposal param-change <프로포절/파일/경로/프로포절.json> \
  --from=<키_명칭(key_name)> \
  --chain-id=<체인_아이디(chain_id)>
```
...
```bash
ochaind query gov proposal <프로포절_ID(proposal_id)>
```

모든 프로포절에 대한 조회를 하기 위해서는:

```bash
ochaind query gov proposals
```
...
```bash
ochaind query gov proposer <프로포절_ID(proposal_id)>
```
...
```bash
ochaind tx gov deposit <프로포절_ID(proposal_id)> "10000000uatom" \
  --from=<키_명칭(name)> \
...
```bash
ochaind query gov deposits <프로포절_ID(proposal_id)>
```

특정 주소에 대한 보증금은 다음과 같이 확인하실 수 있습니다:

```bash
ochaind query gov deposit <프로포절_ID(proposal_id)> <보증금_제공자_주소(depositor_address)>
```
...
```bash
ochaind tx gov vote <프로포절_ID(proposal_id)> <Yes/No/NoWithVeto/Abstain(표 선택)> \
  --from=<키_명칭(key_name)> \
  --chain-id=<체인_아이디(chain-id)>
```
...
```bash
ochaind query gov vote <프로포절_ID(proposal_id)> <투표자_주소(voter_address)>
```
과거 프로포절에 대한 표 정보를 확인하기 위해서는:

```bash
ochaind query gov votes <프로포절_ID(proposal_id)>
```
...
```bash
ochaind query gov tally <프로포절_ID(proposal_id)>
```
...
```bash
ochaind query gov param voting
ochaind query gov param tallying
ochaind query gov param deposit
```
...
```bash
ochaind query distribution params
```
...
```bash
ochaind query distribution outstanding-rewards
```
...
```bash
ochaind query distribution commission <검증인_주소(validator_address)>
```
...
```bash
ochaind query distribution slashes <검증인_주소(validator_address)> <시작_블록_높이(start_height)> <끝_블록_높이(end_height)>
```
...
```bash
ochaind query distribution rewards <위임자_주소(delegator_address)> <검증인_주소(validator_address)>
```
...
```bash
ochaind query distribution rewards <위임자_주소(delegator_address)>
```
...
```bash
ochaind keys show --address p1p2p3
```

멀티시그 주소의 투표 통과 기준, pubkey 투표자 그리고 각자 투표권 부게를 확인하기 위해서는 JSON 아웃풋을 확인하거나 `--show-multisig` 플래그를 사용하실 수 있습니다:

```bash
ochaind keys show p1p2p3 -o json

ochaind keys show p1p2p3 --show-multisig
```

위 주소를 기반으로 멀티시그 트랜잭션을 생성하는 과정의 첫 단계는 다음과 같습니다:

```bash
ochaind tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 10000000uatom \
...
```bash
ochaind tx broadcast signedTx.json
```

## Shell 완료 스크립트

흔히 사용되는 `Bash`와 `Zsh` 같은 UNIX의 완료 스크립트(completion script)는 `completion` 명령어를 사용해 생성될 수 있습니다. 이 명령은 `ochaind`와 `ochaind`에서 사용 가능합니다.

`Bash` 완료 스크립트를 생성하기 위해서는 다음 명령어를 실행하세요:

```bash
ochaind completion > gaiad_completion
ochaind completion > gaiacli_completion
```

`Zsh` 완료 스크립트를 생성하기 위해서는 다음 명령어를 실행하세요:

```bash
ochaind completion --zsh > gaiad_completion
ochaind completion --zsh > gaiacli_completion
```
