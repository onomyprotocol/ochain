...
+ `cosmos` 
	+ 从通过`ochaind keys add`生成的账户私钥中产生
	+ 用于接收资金
...
+ `cosmosvalconspub`
	+ 在使用`ochaind init`创建节点时生成
	+ 使用`ochaind tendermint show-validator`获得该值
...
```bash
ochaind keys show <account_name>
```
...
在广播交易时，`ochaind`接受`--broadcast-mode`标识。 这个标识的值可以是`sync`（默认值）、`async`或`block`，其中`sync`使客户端返回 CheckTx 响应，`async`使客户端立即返回，而`block`使得 客户端等待 tx 被提交（或超时）。
...
```bash
ochaind tx send ... --fees=50000uatom
```
...
```bash
ochaind query account <account_cosmos>
```
...
```bash
ochaind tx send <destination_cosmos> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> 
```
...
```bash
ochaind query account <account_cosmos>
ochaind query account <destination_cosmos>
```

你还可以使用`--block`标识查询在特定高度区块下你的余额：

```bash
ochaind query account <account_cosmos> --block=<block_height>
```

你可以通过在命令行中附加`--dry-run`标识来模拟交易而不实际广播它：

```bash
ochaind tx send <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> \
  --dry-run
```
...
```bash
ochaind tx sign --validate-signatures signedSendTx.json
```
...
```bash
ochaind tx slashing unjail --from <validator-operator-addr>
```
...
```bash
ochaind query slashing signing-info <validator-pubkey>
```
...
```bash
ochaind query staking validator <account_cosmosval>
```
...
```bash
ochaind query staking delegation <delegator_addr> <validator_addr>
```

或者你想查看所有当前的委托：

```bash
ochaind query staking delegations <delegator_addr>
```
...
```bash
ochaind query staking unbonding-delegation <delegator_addr> <validator_addr>
```

或者你可以查看当前你所有的unbonding-delegation:

```bash
ochaind query staking unbonding-delegations <account_cosmos>
```

此外，你可以从特定验证人获取所有unbonding-delegation：

```bash
ochaind query staking unbonding-delegations-from <account_cosmosval>
```
...
```bash
ochaind query staking redelegation <delegator_addr> <src_val_addr> <dst_val_addr>
```

或者，如果你可以检查所有当前的unbonding-delegation：

```bash
ochaind query staking redelegations <account_cosmos>
```

此外，你可以查询某个特定验证人的所有转出的重新绑定：

```bash
ochaind query staking redelegations-from <account_cosmosval>
```
...
```bash
ochaind query delegations-to <account_cosmosval>
```
...
```bash
ochaind tx gov submit-proposal param-change <path/to/proposal.json> \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
ochaind query gov proposal <proposal_id>
```
...
```bash
ochaind query gov proposer <proposal_id>
```
...
```bash
ochaind tx gov deposit <proposal_id> "10000000uatom" \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
ochaind query gov deposits <proposal_id>
```

你还可以查询特定地址提交的存入金：

```bash
ochaind query gov deposit <proposal_id> <depositor_address>
```
...
```bash
ochaind tx gov vote <proposal_id> <Yes/No/NoWithVeto/Abstain> \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
ochaind query gov vote <proposal_id> <voter_address>
```

你还可以查询提交给所有此前投给指定提案的投票：

```bash
ochaind query gov votes <proposal_id>
```
...
```bash
ochaind query gov tally <proposal_id>
```
...
```bash
ochaind query distribution commission <validator_address>
```
...
```bash
ochaind query distribution slashes <validator_address> <start_height> <end_height>
```
...
```bash
ochaind query distribution rewards <delegator_address> <validator_address>
```
...
```bash
ochaind query distribution rewards <delegator_address>
```
...
可以通过完全命令生成主流的UNIX shell解释器（如`Bash`和`Zsh`）的`completion`命令，该命令可用于`ochaind`和`ochaind`。
